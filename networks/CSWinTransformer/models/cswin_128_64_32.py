...
class CSWinTransformer(nn.Module):
    ...
    def __init__(self, img_size=96, patch_size=16, in_chans=1, num_classes=1000, embed_dim=96, depth=[2,2,6,2], split_size = [3,5,7],
        ...
        self.stage1_conv_embed = nn.Sequential(
            nn.Conv3d(in_chans, embed_dim, 5, 2, 2),
            Rearrange('b c d h w -> b (d h w) c', d = img_size//2, h = img_size//2, w = img_size//2),
        ...
        self.stage5 = nn.ModuleList(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[4], reso=img_size//32, mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale, split_size=split_size[-1],
                drop=drop_rate, attn_drop=attn_drop_rate,
                drop_path=dpr[np.sum(depth[:-1])+i], norm_layer=norm_layer)
                # drop_path=dpr[np.sum(depth[:-1])+i], norm_layer=norm_layer, last_stage=True)
    ...
    def forward_features(self, x):
        ...
        for blk in self.stage1:
            if self.use_chk:
                ...
                x = blk(x)
        for pre, blocks in zip([self.merge1, self.merge2, self.merge3, self.merge4], 
                               [self.stage2, self.stage3, self.stage4, self.stage5]):
            x = pre(x)
            for blk in blocks:
                if self.use_chk:
                    x = checkpoint.checkpoint(blk, x)
    ...
    def forward(self, x):
        ...
        B, N, C = x.shape
        D = H = W = int(np.ceil(math.pow(N, 1/3)))      
        tx = x.transpose(-2,-1).contiguous().view(B, C, D, H, W)
        x0_out = self.proj_out(tx, normalize)
...
def CSWin_3_12211_tiny_96(pretrained=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=48, depth=[2,4,32,2],
        split_size=[1,2,3,3], num_heads=[2,4,8,16,32], mlp_ratio=4., **kwargs)
    model.default_cfg = default_cfgs['cswin_224']
    return model

@register_model
def CSWin_8_12211_tiny_128(pretrained=False, **kwargs):
    model = CSWinTransformer(img_size=128, patch_size=4, embed_dim=48, depth=[2,4,32,2],
        split_size=[1,2,4,4], num_heads=[2,4,8,16,32], mlp_ratio=4., **kwargs)
    model.default_cfg = default_cfgs['cswin_224']
    return model
