...
class LePEAttention(nn.Module):
    def __init__(self, dim, resolution, idx, split_size=7, dim_out=None, num_heads=8, attn_drop=0., proj_drop=0., qk_scale=None):
        ...
        if idx == -1:
            # no use attentation
            H_sp, W_sp, D_sp = self.resolution, self.resolution, self.resolution
        elif idx == 0:
            # only use x-axis attentation
            H_sp, W_sp, D_sp = self.resolution, self.split_size, self.split_size
        elif idx == 1:
            # only use y-axis attentation
            W_sp, H_sp, D_sp = self.resolution, self.split_size, self.split_size
        elif idx == 2:
            # only use z-axis attentation
            D_sp, H_sp, W_sp = self.resolution, self.split_size, self.split_size  
        else:
    ...
    def im2cswin(self, x):
        B, N, C = x.shape
        D = H = W = int(np.ceil(math.pow(N, 1/3)))
...
class Merge_Block(nn.Module):
    def __init__(self, dim, dim_out, norm_layer=nn.LayerNorm):
        ...
        self.norm_save = norm_layer(int(dim_out/2))
        self.norm = norm_layer(dim_out)

    def forward(self, x):
        B, new_DHW, C = x.shape
        D = H = W = int(np.ceil(math.pow(new_DHW, 1/3)))
        x = x.transpose(-2, -1).contiguous().view(B, C, D, H, W)
        
        # save res
        n, ch, d, h, w = x.shape
        save = rearrange(x, "n c d h w -> n d h w c")
        save = F.layer_norm(save, [ch])
        save = rearrange(save, "n d h w c -> n c d h w")
...
class CSWinTransformer(nn.Module):
    ...
    """
    def __init__(self, img_size=96, patch_size=16, in_chans=1, num_classes=1000, embed_dim=96, depth=[2,2,6,2], split_size = [3,5,7],
                 num_heads=12, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, use_chk=False, first_stage_divide=2):
        super().__init__()
        self.use_chk = use_chk
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        heads=num_heads
        self.first_stage_divide = first_stage_divide
        
        
        self.stage1_conv_embed = nn.Sequential(
            nn.Conv3d(in_chans, embed_dim, self.first_stage_divide+3, self.first_stage_divide, 2),
            Rearrange('b c d h w -> b (d h w) c', d = img_size//self.first_stage_divide, h = img_size//self.first_stage_divide, w = img_size//self.first_stage_divide),
        ...
        self.stage1 = nn.ModuleList([
            CSWinBlock(
                dim=curr_dim, num_heads=heads[0], reso=img_size//self.first_stage_divide, mlp_ratio=mlp_ratio,
        ...
        self.stage2 = nn.ModuleList(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[1], reso=img_size//(self.first_stage_divide*2), mlp_ratio=mlp_ratio,
        ...
        temp_stage3.extend(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[2], reso=img_size//(self.first_stage_divide*4), mlp_ratio=mlp_ratio,
        ...
        self.stage4 = nn.ModuleList(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[3], reso=img_size//(self.first_stage_divide*8), mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale, split_size=split_size[-1],
                drop=drop_rate, attn_drop=attn_drop_rate,
                drop_path=dpr[np.sum(depth[:3])+i], norm_layer=norm_layer)
                # drop_path=dpr[np.sum(depth[:-1])+i], norm_layer=norm_layer, last_stage=True)
            for i in range(depth[3])])
        
        # new add
        if self.first_stage_divide==2:
            self.merge4 = Merge_Block(curr_dim, curr_dim*2)
            curr_dim = curr_dim*2
            self.stage5 = nn.ModuleList(
                [CSWinBlock(
                    dim=curr_dim, num_heads=heads[4], reso=img_size//(self.first_stage_divide*16), mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias, qk_scale=qk_scale, split_size=split_size[-1],
                    drop=drop_rate, attn_drop=attn_drop_rate,
                    drop_path=dpr[np.sum(depth[:-1])+i], norm_layer=norm_layer)
                    # drop_path=dpr[np.sum(depth[:-1])+i], norm_layer=norm_layer, last_stage=True)
    ...
    def forward(self, x):
        ...
        if self.first_stage_divide==2:
            layers01 = [self.merge1, self.merge2, self.merge3, self.merge4]
            layers02 = [self.stage2, self.stage3, self.stage4, self.stage5]
        elif self.first_stage_divide==4:
            layers01 = [self.merge1, self.merge2, self.merge3]
            layers02 = [self.stage2, self.stage3, self.stage4]
            
        for pre, blocks in zip(layers01, layers02):
            x, save = pre(x) #get merge result
            hidden_states_out.append(save)
            # print('*** hidden_states_out: ', save.shape)
        ...
        x = self.norm(x)
        save = self.proj_out(x) # [-1, 27, 1024] -> [-1, 1024, 3, 3, 3]
        hidden_states_out.append(save)
        # print('*** hidden_states_out: ', save.shape)
...
@register_model
def CSWin_3_12211_tiny_96(pretrained=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=64, depth=[2,4,32,2],
        split_size=[1,2,3,3], num_heads=[2,4,8,16,32], mlp_ratio=4., **kwargs)
