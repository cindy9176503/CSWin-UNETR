...
class LePEAttention(nn.Module):
    ...
    def im2cswin(self, x):
        print('LePEAttention im2cswin: ', x.shape)
        B, N, C = x.shape
        H = W = int(np.sqrt(N))
    ...
    def get_lepe(self, x, func):
        print('LePEAttention get_lepe: ', x.shape)
        B, N, C = x.shape
        H = W = int(np.sqrt(N))
        x = x.transpose(-2,-1).contiguous().view(B, C, H, W)
    ...
    def forward(self, qkv):
        print('LePEAttention forword: ', qkv.shape)
        """
        x: B L C
        """
...
class CSWinBlock(nn.Module):
    ...
    def forward(self, x):
        print('CSWinBlock forword: ', x.shape)
        """
        x: B, H*W, C
        """
...
class CSWinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        print('CSWinTransformer forward_features:', x.shape)
        B = x.shape[0]
...
@register_model
def CSWin_64_12211_tiny_224_ori(pretrained=False, **kwargs):
...
@register_model
def CSWin_96_24322_base_224_ori(pretrained=False, **kwargs):
